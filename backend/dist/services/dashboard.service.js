    ...
    async getDashboardData() {
        ...
        try {
            ...
            const expiringSoonResult = db
                .prepare(`SELECT COUNT(*) as count FROM inventory_items WHERE expiry_date <= date('now', '+90 day') AND status = 'Normal'`)
                .get();
            ...
            const recentActivityResult = db
                .prepare('SELECT id, change_description as description, created_at as timestamp FROM audit_log ORDER BY created_at DESC LIMIT 5')
                .all();
            return {
