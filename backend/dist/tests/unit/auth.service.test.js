...
describe('AuthService', () => {
    ...
    beforeEach(() => {
        ...
        jsonwebtoken_1.default.verify.mockReturnValue({ userId: 1, role: 'Manager' });
    ...
    describe('hashPin', () => {
        it('hashes PIN with bcrypt', async () => {
            ...
            expect(bcrypt_1.default.hash).toHaveBeenCalledWith('5624', 10);
    ...
    describe('verifyPin', () => {
        it('verifies PIN with bcrypt', async () => {
            ...
            expect(bcrypt_1.default.compare).toHaveBeenCalledWith('5624', 'hashed_pin');
    ...
    describe('login', () => {
        ...
        it('throws InternalError on unexpected failures', async () => {
            ...
            await expect(authService.login('5624')).rejects.toBeInstanceOf(errors_1.InternalError);
        });
    });
    ...
    describe('verifyToken', () => {
        it('successfully verifies and decodes a valid token', () => {
            jsonwebtoken_1.default.verify.mockReturnValue({ userId: 7, role: 'Staff', exp: 1234567890 });
            const payload = authService.verifyToken('valid_token');
            expect(payload.userId).toBe(7);
            expect(payload.role).toBe('Staff');
            expect(jsonwebtoken_1.default.verify).toHaveBeenCalledWith('valid_token', 'test_secret');
        });
        it('throws AuthenticationError for expired token', () => {
            ...
            expect(() => authService.verifyToken('expired_token')).toThrow(errors_1.AuthenticationError);
        });
        it('throws AuthenticationError for tampered/invalid signature', () => {
            ...
            expect(() => authService.verifyToken('tampered_token')).toThrow(errors_1.AuthenticationError);
        });
    });
    describe('refreshAccessToken', () => {
        ...
        it('throws AuthenticationError for non-existent refresh token', async () => {
            ...
            await expect(authService.refreshAccessToken('invalid_token')).rejects.toBeInstanceOf(errors_1.AuthenticationError);
        });
        it('throws AuthenticationError and deletes expired refresh token', async () => {
            ...
            await expect(authService.refreshAccessToken('expired_refresh_token')).rejects.toBeInstanceOf(errors_1.AuthenticationError);
        ...
        it('throws AuthenticationError for revoked refresh token', async () => {
            ...
            await expect(authService.refreshAccessToken('revoked_token')).rejects.toBeInstanceOf(errors_1.AuthenticationError);
        });
    });
    describe('revokeRefreshToken', () => {
        ...
        it('throws InternalError on database failure', async () => {
            ...
            prisma.refreshToken.update.mockRejectedValue(new Error('DB failure'));
            await expect(authService.revokeRefreshToken('token')).rejects.toBeInstanceOf(errors_1.InternalError);
        });
    });
    describe('cleanupExpiredTokens', () => {
        ...
        it('returns 0 on database failure', async () => {
            prisma.refreshToken.deleteMany.mockRejectedValue(new Error('DB failure'));
            const count = await authService.cleanupExpiredTokens();
            expect(count).toBe(0);
        });
    });
