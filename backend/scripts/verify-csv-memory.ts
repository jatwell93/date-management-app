...
async function generateTestCSV(lineCount: number): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    stream.on('finish', () => {
      ...
      console.log(`Generated ${(stats.size / 1024 / 1024).toFixed(2)} MB file in ${Date.now() - startTime}ms`);
      resolve(filePath);
    });
...
async function processCSVWithMemoryTracking(filePath: string, snapshotInterval: number = 1000): Promise<MemorySnapshot[]> {
  ...
  for await (const row of parser) {
    ...
    if (rowCount % snapshotInterval === 0) {
      ...
      if (rowCount % 10000 === 0) {
        ...
        console.log(`  Processed ${rowCount.toLocaleString()} rows - Heap: ${mem.heapUsedMB.toFixed(2)} MB, RSS: ${mem.rssMB.toFixed(2)} MB`);
      }
    }
  }
