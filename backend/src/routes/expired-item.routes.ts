...
router.post("/process", authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  if (inventoryItemId === undefined || inventoryItemId === null ||
    typeof inventoryItemId !== 'number' || Number.isNaN(inventoryItemId) || inventoryItemId < 1) {
    return res
      .status(400)
      .json({ message: "Missing or invalid required field: inventoryItemId" });
  }

  if (!action || (action !== 'sold_through' && action !== 'expired')) {
    return res
      .status(400)
      .json({ message: "Action must be either 'sold_through' or 'expired'" });
  }

  // Validate unitsDiscarded if action is 'expired'
  if (action === 'expired') {
    if (unitsDiscarded === undefined || unitsDiscarded === null ||
      typeof unitsDiscarded !== 'number' || Number.isNaN(unitsDiscarded) || unitsDiscarded <= 0) {
      return res
        .status(400)
...
router.get("/reports/expired-losses", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    // Get both reports
