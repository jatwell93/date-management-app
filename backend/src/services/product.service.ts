...
function formatErrorMessage(rowNumber: number, field: string, value: string, expected: string): string {
...
export class ProductService {
  ...
  private async processXLSXUpload(filePath: string): Promise<{ imported: number; updated: number; errors: string[] }> {
    const errors: string[] = [];
    let imported = 0;
    let updated = 0;
    ...
    try {
      ...
      const skuColIndex = this.findColumnIndexByAlternatives(headers as (string | null | undefined)[], ['SKU', 'Item Code', 'Reorder Number', 'Product Code', 'Item Number']);
      const nameColIndex = this.findColumnIndexByAlternatives(headers as (string | null | undefined)[], ['Name', 'Item Description', 'Product Name', 'Description', 'Item Name']);
      const costColIndex = this.findColumnIndexByAlternatives(headers as (string | null | undefined)[], ['Cost', 'Cost Price', 'Unit Cost', 'Cost ex', 'Price', 'Unit Price', 'Cost inc', 'Selling Price', 'Retail Price']);
      const barcodeColIndex = this.findColumnIndexByAlternatives(headers as (string | null | undefined)[], ['Barcode', 'Alias', 'EAN', 'UPC', 'GTIN', 'Product Barcode', 'Barcode Number']);
      ...
      const allowedHeaders = [
        ...
        ...['Cost', 'Cost Price', 'Unit Cost', 'Cost ex', 'Price', 'Unit Price', 'Cost inc', 'Selling Price', 'Retail Price'],
        ...['Barcode', 'Alias', 'EAN', 'UPC', 'GTIN', 'Product Barcode', 'Barcode Number']
      ].filter((header): header is string => header !== null && header !== undefined).map(header => header.toLowerCase());
        ...
        try {
          ...
          if (existingProduct) {
            ...
            try {
              ...
              errors.push(`Row ${recordCount}: Failed to update existing product (SKU: ${sku}) - ${(updateError as Error).message}`)
            }
          } else {
            // Create new product
            try {
              ...
              errors.push(`Row ${recordCount}: Failed to create new product (SKU: ${sku}) - ${(createError as Error).message}`);
            }
          }
        } catch (error) {
  ...
  private findColumnIndexByAlternatives(headers: (string | null | undefined)[], alternatives: string[]): number | null {
  ...
  private async validateCSVStructure(filePath: string): Promise<{ isValid: boolean; errors: string[] }> {
    const errors: string[] = [];
    let isValid = true;
  ...
  private findColumnByAlternatives(row: any, alternatives: string[]): string | null {
    ...
    for (const alt of alternatives) {
      ...
      const foundHeader = headers.find(header => 
        header.toLowerCase() === alt.toLowerCase()
      );
      if (foundHeader) {
        return foundHeader;
      }
