...
// Helper to generate large CSV files
function generateLargeCSV(lineCount: number): string {
  const tempDir = os.tmpdir();
  const filePath = path.join(tempDir, `test-csv-${Date.now()}-${lineCount}.csv`);
  ...
  // Return the path after the stream is done
...
describe('CSV Parser Integration', () => {
  ...
  describe('Basic Integration', () => {
    ...
    it('should handle duplicate SKUs by updating existing records', async () => {
      ...
      expect(product?.costPrice).toBe(20.00);
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle various cost formats in integration', async () => {
      ...
      expect(products[1].costPrice).toBe(15.50);
      expect(products[2].costPrice).toBe(20.00);
