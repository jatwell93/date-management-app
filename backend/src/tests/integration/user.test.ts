...
describe('User API Integration Tests', () => {
  ...
  describe('POST /auth/login', () => {
    it('should login successfully with valid PIN', async () => {
      ...
      const response = await request(app).post('/auth/login').send({ pin: '5624' }).expect(200);
    ...
    it('should return 401 for invalid PIN', async () => {
      ...
      await request(app).post('/auth/login').send({ pin: 'wrongpin' }).expect(401);
    });
  });
  ...
  describe('GET /users (Manager only)', () => {
    ...
    it('should return all users when accessed by manager', async () => {
      ...
      const authResponse = await request(app).post('/auth/login').send({ pin: '5624' });
  ...
  describe('DELETE /users/:id (Manager only)', () => {
    it('should delete a user when accessed by manager', async () => {
      ...
      await request(app).delete('/users/2').set('Authorization', `Bearer ${token}`).expect(200);
    });
  });
});
