...
describe('InventoryService - Markdown Calculation Alignment', () => {
  ...
  describe('calculateMarkdownStatus methods should be consistent', () => {
    const testCases = [
      ...
      {
        name: '30 days from expiry (Markdown 3)',
        expiryDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Markdown 3',
      },
      {
        name: '45 days from expiry (Markdown 2)',
        expiryDate: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Markdown 2',
      },
      {
        name: '60 days from expiry (Markdown 2)',
        expiryDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Markdown 2',
      },
      {
        name: '75 days from expiry (Markdown 1)',
        expiryDate: new Date(Date.now() + 75 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Markdown 1',
      },
      {
        name: '90 days from expiry (Markdown 1)',
        expiryDate: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Markdown 1',
      },
      {
        name: '91 days from expiry (Normal)',
        expiryDate: new Date(Date.now() + 91 * 24 * 60 * 60 * 1000).toISOString(),
        expected: 'Normal',
      },
      {
  ...
  it('should have accessible markdown thresholds', () => {
    ...
    expect(InventoryService['MARKDOWN_THRESHOLDS'].markdown3).toBe(30);
    expect(InventoryService['MARKDOWN_THRESHOLDS'].markdown2).toBe(60);
    expect(InventoryService['MARKDOWN_THRESHOLDS'].markdown1).toBe(90);
  });
});
