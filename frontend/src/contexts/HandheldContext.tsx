...
export const HandheldProvider: React.FC<HandheldProviderProps> = ({ children }) => {
  ...
  const [syncStrategy, setSyncStrategyState] = useState<SyncStrategy>(() => {
    const savedStrategy = localStorage.getItem(STORAGE_KEYS.SYNC_STRATEGY) as SyncStrategy;
    return savedStrategy && Object.values(SYNC_STRATEGIES).includes(savedStrategy)
      ? savedStrategy
      : DEFAULTS.SYNC_STRATEGY;
  });
