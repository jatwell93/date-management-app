...
jest.mock('../../components/Scanner', () => ({
  Scanner: ({ onScan }: { onScan: (val: string) => void }) => (
    <div data-testid="mock-scanner">
      ...
      />
      <button data-testid="trigger-scan" onClick={() => onScan('1234567890')}>Scan Barcode</button>
      <button data-testid="trigger-sku-scan" onClick={() => onScan('123456')}>Scan SKU</button>
    </div>
  ),
}));
...
describe('ScanPage Integration', () => {
    ...
    beforeEach(() => {
        ...
        (apiService.get as jest.Mock).mockImplementation((url) => {
            ...
            // Fail safe
            if (url.includes('/products/by-sku/1234567890')) return Promise.reject(new Error('Not found'));
    ...
    it('displays product details after scanning a valid barcode (>8 chars)', async () => {
        ...
        await waitFor(() => {
             expect(apiService.get).toHaveBeenCalledWith(expect.stringContaining('/products/by-barcode/1234567890'), mockToken);
        });
    ...
    it('displays product details after scanning a valid SKU (<=8 chars)', async () => {
        ...
        await waitFor(() => {
             expect(apiService.get).toHaveBeenCalledWith(expect.stringContaining('/products/by-sku/123456'), mockToken);
        });
