...
export const getExpiredItems = async (token: string | null): Promise<ExpiredItem[]> => {
  try {
    ...
    const response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/expired-items`, {
      method: 'GET',
      headers: {
...
export const processExpiredItem = async (request: ProcessExpiredItemRequest, token: string | null): Promise<ExpiredItemTransaction> => {
  try {
    if (!token) {
      throw new Error('Authentication token not found');
    }

    const response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/expired-items/process`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
...
export const getExpiredLossesReport = async (token: string | null): Promise<{
  ...
  try {
    ...
    const response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/expired-items/reports/expired-losses`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch expired losses report: ${response.status} ${response.statusText}`);
    }
