...
global.fetch = jest.fn((url: RequestInfo | URL) => {
  ...
  if (urlString.includes('/products?barcode=')) {
    ...
    return Promise.resolve({
      ok: true,
      json: () => Promise.resolve({ message: 'Inventory item added successfully!' }),
    } as Response);
...
describe('ScanPage', () => {
  ...
  it('displays an error message if token is missing', async () => {
    ...
    expect(await screen.findByText(/Authentication token is missing./i)).toBeInTheDocument();
  });
  ...
  it('adds a new product and then shows inventory details form', async () => {
    ...
    expect(
      await screen.findByText(/New product added successfully! Now add inventory details./i),
    ).toBeInTheDocument();
  ...
  it('saves inventory item to offline storage when offline', async () => {
    ...
    expect(
      await screen.findByText(/Offline: Inventory item saved for synchronization./i),
    ).toBeInTheDocument();
