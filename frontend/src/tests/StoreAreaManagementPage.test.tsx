  ...
  if (urlString.includes('/store-areas')) {
    if (options?.method === 'POST') {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ id: 3, name: 'New Area', last_checked: null }),
      } as Response);
    } else if (options?.method === 'PUT') {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ message: 'Store area updated successfully!' }),
      } as Response);
    } else if (options?.method === 'DELETE') {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ message: 'Store area deleted successfully!' }),
      } as Response);
    } else {
      // GET request
      return Promise.resolve({
