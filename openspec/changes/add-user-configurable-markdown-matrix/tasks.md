...
- [ ] **DEFERRED** (user decision): backend Express importer (`product-import.helpers.ts` / `product.service.ts`) still folds Retail/Selling Price into cost. Live path is workers. See follow-up below.
...
1. Backend Express importer retail capture (parity with workers path).
2. Scan page + detailed expiry report price displays consume the org matrix.
3. v2: snapshot resolved markdown price for frozen history; configurable day windows / >3 bands.
