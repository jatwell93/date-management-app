...
- [x] 7.8 Run existing test suites: `npm test` passes with no regressions. Update any tests that reference old `inventory-*` class names to use new semantic token classes.
  - 2026-05-19: `npm test --prefix frontend -- --watchAll=false` passed 45/45 suites and 372/372 tests. Console output still includes existing test-environment warnings for act wrapping, chart context, and expected sync failure logging.
