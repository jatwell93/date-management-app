...
- [ ] 0.0 Verify token source artifacts: obtain official PharmIQ design token artifacts (JSON/CSS). Document source location and checksum. If artifacts don't exist yet, create placeholder `design-tokens.json` with values from spec.md and note as "pending source verification".
- [ ] 0.1 Create `frontend/src/theme/` directory and add `design-tokens.json` containing all raw PharmIQ brand token values: colors (teal `#0F766E`, sky blue `#0EA5E9`, amber `#D97706`, green `#10B981`, red `#DC2626`, navy `#0F172A`, sky-light `#F0F9FF`), typography scale, spacing (8px base grid), radius, elevation, and motion values. Also generate `design-tokens.css` exporting the same values as CSS custom properties for direct CSS consumption. Verify both files' checksums match against source token artifacts.
- [ ] 0.2 Create `frontend/src/theme/tokens.ts` that imports from `design-tokens.json` and exports typed token constants for use in JS/TS code (colors, spacing, typography, radius, elevation, motion as named exports).
...
- [ ] 0.4 Refactor `frontend/tailwind.config.js`: replace the `inventory.*` color scale system with semantic token theme extensions sourced from `semantic-tokens.ts`. Add `semantic` color namespace (e.g., `bg-semantic-primary`, `text-semantic-text-primary`). Extend with font-family tokens for owner-facing (Space Grotesk + Inter) and worker-facing (Plus Jakarta Sans + DM Sans) contexts. Verify build passes with zero CSS warnings.
- [ ] 0.5 Deduplicate and refactor `frontend/src/globals.css` and `frontend/src/index.css`: regenerate CSS custom properties from the token pipeline rather than hardcoded HSL values. Update `:root` and `.dark` blocks to derive from `design-tokens.json`. Update `body` font-family from system stack to brand default (Inter). Remove duplicate custom property blocks.
- [ ] 0.6 Create `frontend/src/theme/SEMANTIC_COLORS_REFERENCE.md` documenting each semantic token's name, raw brand token source, intended usage context, and dark-mode variant.
- [ ] 0.7 Integrate `design-tokens.css` (from 0.1) and semantic token CSS custom properties (`--semantic-primary`, `--semantic-warning`, etc.) into `globals.css` or as a separate import in `index.css`. Ensure both light and dark mode variants are generated. Import order: `design-tokens.css` → `semantic` layer → component styles.
- [ ] 0.9 Define migration strategy: decide whether to remove `inventory-*` Tailwind colors entirely (breaking change) or deprecate with warning. Document decision in `frontend/TOKEN_MIGRATION_STRATEGY.md`. If removing, add codemod task to auto-replace `inventory-*` with `semantic-*` across codebase.
...
- [ ] 5.1 Add brand font packages to `frontend/package.json`: `@fontsource/space-grotesk`, `@fontsource/inter`, `@fontsource/plus-jakarta-sans`, `@fontsource/dm-sans` (or equivalent self-hosted font imports). Configure font loading with `font-display: swap` to avoid FOIT.
- [ ] 5.2 Implement audience-context font application: owner-facing surfaces (Dashboard, Reports, Settings, User Management) use Space Grotesk headings + Inter body; worker-facing surfaces (Scan, Expiry workflows, handheld) use Plus Jakarta Sans headings + DM Sans body. Apply via CSS custom properties or Tailwind font-family tokens scoped by context class (e.g., `.owner-context`, `.worker-context`).
- [ ] 5.3 Conduct messaging audit: grep all user-facing text strings in owner vs worker surfaces. Owner surfaces must emphasize margins/outcomes language ("Recover dead stock", "Smart Ops. Better Margins."). Worker surfaces must use direct utility language ("Know what you're owed", "Scan to verify"). Document findings in `docs/voice-audit.md`. Fix any mismatched language.
- [ ] 5.4 Verify typography in browser DevTools: load dashboard (owner context) → confirm Space Grotesk + Inter; load scanner screen (worker context) → confirm Plus Jakarta Sans + DM Sans.
