...
3. **Sold-through reappearing** — `getDetailedExpiryReport` had no terminal-status filter, and the two
   backends snapshot *different* post-disposition statuses (`workers/src/database.ts:798`
...
time, after they ship. This change addresses the *class*, not another instance.
...
- **Reuse both existing test harnesses.** The conformance suite drives the *real* report/disposition
...
3. Add a conformance suite that seeds identical data into the pglite (Postgres) and better-sqlite3
   ...
   so the NULLS-ordering class is caught automatically.
4. Add a regression case per past defect (zeros, threshold split, sold-through reappear, NULLS order)
   so each historical bug is now permanently fenced.

Phase 3 — Make the convention stick
5. Add a "dual-backend parity" Golden Rule to `openspec/project.md` and a checklist line to AGENTS.md:
   any logic present in both backends sources shared constants and has a conformance test.
6. Run completion checks (`npm run lint`, backend jest, `npm run test:db`, `openspec validate --all`).
