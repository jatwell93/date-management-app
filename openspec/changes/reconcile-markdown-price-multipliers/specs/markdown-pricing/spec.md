...
from expiry have no markdown price. Expired stock (on or past its used-by date) is written off, not
marked down, and SHALL have no markdown price — the price the system returns MUST agree with the
markdown status it reports for the same item.
...
#### Scenario: Expired stock has no markdown price

- **GIVEN** an item on or past its used-by date (0 or fewer days to expiry)
- **WHEN** its markdown price is computed on the backend
- **THEN** no markdown price is returned
- **AND** this agrees with the item's markdown status being reported as Expired
