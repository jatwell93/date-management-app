...
exports.ALERT_THRESHOLDS = exports.TIER_PRICES = exports.TIER_LIMITS = exports.AVAILABLE_FEATURES = exports.BillingCycle = exports.SubscriptionStatus = void 0;
...
function validateAlertThresholds(thresholds) {
    ...
    if (validated.trialConversionRateMin < 0 || validated.trialConversionRateMin > 100) {
        throw new Error(`trialConversionRateMin must be between 0 and 100, got ${validated.trialConversionRateMin}`);
    }
    if (validated.webhookFailureRateMax < 0 || validated.webhookFailureRateMax > 100) {
        throw new Error(`webhookFailureRateMax must be between 0 and 100, got ${validated.webhookFailureRateMax}`);
    }
    if (validated.paymentFailureRateMax < 0 || validated.paymentFailureRateMax > 100) {
        throw new Error(`paymentFailureRateMax must be between 0 and 100, got ${validated.paymentFailureRateMax}`);
    }
