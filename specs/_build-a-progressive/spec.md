...
- Q: What are the distinct states an inventory item can be in as it approaches its expiry date? This clarifies the data model and business logic. → A: Four states: 'Normal', 'Markdown 1' (3 months), 'Markdown 2' (2 months), 'Markdown 3' (1 month), and 'Expired'.
- Q: What is the process for handling products that are discontinued and should no longer be tracked in the app? → A: Products are never removed; they just won't appear in new CSV uploads.
- Q: What is the maximum acceptable time from scanning a barcode to the app being ready for expiry date entry? → A: 2-4 seconds.
...
- **Performance**: The application must be responsive, with barcode scans resolving and the UI being ready for date entry within 2-4 seconds.

### Out of Scope
- **Product Deletion**: The system will not include a feature for explicitly deleting or deactivating products. Discontinued items will simply be omitted from future CSV uploads.
