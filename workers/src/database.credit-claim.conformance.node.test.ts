...
// migration 015 columns the query touches.
...
function sqliteClaimablePool(db: import('better-sqlite3').Database, org: string): ClaimablePoolGroup[] {
  const rows = db
    .prepare(
...
describe('dual-backend claimable-pool conformance', () => {
  ...
  async function seed(scenario: {
    ...
    products: Array<{ id: number; sku: string; cost: number; supplierId: number | null }>;
    writeOffs: Array<{ txId: number; productId: number; units: number; claimed?: boolean; org?: string }>;
  }): Promise<void> {
    for (const s of scenario.suppliers) {
      ...
      sqlite
        .prepare('INSERT INTO suppliers (id, organization_id, name, policy_write_off_qty, policy_credit_qty) VALUES (?, ?, ?, ?, ?)')
    ...
    if (scenario.writeOffs.some((w) => w.claimed) && scenario.suppliers[0]) {
      ...
      sqlite
        .prepare('INSERT INTO credit_claims (id, organization_id, supplier_id, status) VALUES (?, ?, ?, ?)')
        .run(1, ORG, supplierId, 'SENT');
    }
    for (const p of scenario.products) {
      await sql`INSERT INTO products (id, organization_id, barcode, sku, name, cost_price, supplier_id)
                VALUES (${p.id}, ${ORG}, ${p.sku}, ${p.sku}, ${'Item ' + p.sku}, ${p.cost}, ${p.supplierId})`;
      sqlite
        .prepare('INSERT INTO products (id, organization_id, barcode, sku, name, cost_price, supplier_id) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(p.id, ORG, p.sku, p.sku, `Item ${p.sku}`, p.cost, p.supplierId);
    }
    for (const w of scenario.writeOffs) {
      ...
      await sql`INSERT INTO expired_item_transactions (id, organization_id, inventory_item_id, action, units_discarded)
                VALUES (${w.txId}, ${org}, ${w.txId}, ${'expired'}, ${w.units})`;
      sqlite
        .prepare('INSERT INTO inventory_items (id, organization_id, product_id, location_id, status) VALUES (?, ?, ?, ?, ?)')
        .run(w.txId, org, w.productId, 1, 'Expired');
      sqlite
        .prepare('INSERT INTO expired_item_transactions (id, organization_id, inventory_item_id, action, units_discarded) VALUES (?, ?, ?, ?, ?)')
        .run(w.txId, org, w.txId, 'expired', w.units);
      if (w.claimed) {
        ...
        sqlite
          .prepare('INSERT INTO credit_claim_lines (organization_id, claim_id, expired_item_transaction_id, units_claimed) VALUES (?, ?, ?, ?)')
