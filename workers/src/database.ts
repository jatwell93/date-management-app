...
export function createWorkersDatabase(env: Env): Database {
  ...
  return {
    ...
    async findSupplier(organizationId, id): Promise<Supplier | null> {
      const rows = (await sql`
        SELECT id, name,
    ...
    async createSupplier(organizationId, data): Promise<Supplier> {
      const rows = (await sql`
    ...
    async updateSupplier(organizationId, id, data): Promise<Supplier | null> {
      const rows = (await sql`
    ...
    async clearSupplierPolicy(organizationId, id): Promise<Supplier | null> {
      const rows = (await sql`
        UPDATE suppliers SET
    ...
    async listPolicyReview(organizationId, options): Promise<PolicyReviewItem[]> {
    ...
    async bulkLinkProducts(
      ...
      const rows = (await sql`
        WITH requested AS (
          ...
          INSERT INTO brands (
            organization_id, name, source, created_at, updated_at
          )
          ...
          RETURNING p.id, p.barcode, t.id AS brand_id, t.name AS brand_name,
                    t.supplier_id
        ), corrections AS (
          INSERT INTO catalogue_corrections (
      ...
      `) as Array<Record<string, unknown>>;
      const row = rows[0];
      if (brandId != null && row.brandId == null) return { kind: 'BRAND_NOT_FOUND' };
      if (Number(row.requestedCount) !== Number(row.foundCount))
